...
function first(){
  ...
  function second () {
    ...
    //함수안에 함수가 있으면 외부변수를 참조할 수 있음 
    // => outer environment reference : 부모한테서 가져오는것(여기서는 first), 상위 execution context안에 있는 lexical environment
   return x + y
  }

  //함수를 실행한 값이 아니라 함수 자체를 return
  return second
}
//  return x+y에서 retun을 빼면 
// console.log(first())          // seocnd 함수 전체가 찍힘 => 자바스크립트는 함수를 값으로 인식함, 그래서 함수 전체를 값으로 내보냄
// console.log(first()())  // 이렇게 해야 second까지 실행된 값이 나옴 => 커링함수
...
//클로저를 사용하지 않아서 일반함수로 전역이 오염된 버전
// let count = 0;
// function number () {
//   return ++count
// }
...
function number () {
  ...
  function inner() {
    console.log(count)   //여기서는 접근이 가능함
    return ++count
  }
  return inner
}
...
// console.log(count)   //이건 number함수 안에 정의되서 접근이 안됨.
